...
const Invoice = {
    getByUserId: (userId, callback) => {
        const sql = `
            SELECT
                        ...
                        WHERE inv.userid = ? and inv.ispurchase = 0;
        `;
    ...
    markAsPurchased: (invoiceIds, callback) => {
        const sql = "UPDATE invoices SET ispurchase = 1 WHERE id IN (?)";
        db.query(sql, [invoiceIds], callback);
    },

    selectPurchased: (invoiceIds, callback) => {
        const sql = "SELECT * FROM herbalife2.invview WHERE id IN (?)";
        db.query(sql, [invoiceIds], callback);
    }
};
